...
class TestRateLimiter:
    ...
    @pytest.mark.asyncio
    async def test_ip_and_user_counts_are_independent(self):
    ...
    async def test_different_users_independent(self):
        ...
        assert await rl.check_user("alice") is True
        assert await rl.check_user("alice") is False
        assert await rl.check_user("bob") is True  # separate bucket
    ...
    @pytest.mark.asyncio
    async def test_ip_per_ip_limit(self):
        rl = RateLimiter(per_user_limit=100, per_ip_limit=3, window_seconds=60)
        assert await rl.check_ip("1.2.3.4") is True
        assert await rl.check_ip("1.2.3.4") is True
        assert await rl.check_ip("1.2.3.4") is True
        assert await rl.check_ip("1.2.3.4") is False

    @pytest.mark.asyncio
    async def test_evicts_expired_entries_on_check(self):
        ...
        assert await rl.check_user("user") is True
        assert await rl.check_user("user") is True
        assert await rl.check_user("user") is True
        assert await rl.check_user("user") is False  # 4th exceeds limit
